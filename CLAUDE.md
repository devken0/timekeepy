...
`WORK_START_HOUR` is the pivot for everything time-related — `main.py` uses it to choose Time In vs Time Out ([main.py:206-211](main.py#L206-L211)), and the notify wrappers read it at fire time to gate their dialogs. Change `WORK_START_HOUR` and the next day's reminders shift automatically without reinstalling the LaunchAgents.

## Architecture

**`main.py`** is a single linear `main()` function — opens the webcam app and waits for it to quit, grabs the most recent `.jpg` from `SELFIE_DIR`, moves it to `/tmp/<timestamp>.jpg`, then drives Safari through login → action dropdown → file upload → submit → screenshot. Before using the selfie it rejects any whose mtime predates the session start ([main.py:164](main.py#L164)) — guards against uploading a stale photo when no new one was taken. Action is picked by clock time vs `WORK_START_HOUR` (≤ start → Time In, else Time Out). After saving the screenshot it pops the image in Quick Look (`qlmanage -p`) for `SCREENSHOT_PREVIEW_SECONDS` as a non-blank confirmation, auto-dismissing by terminating that process — this replaced an old fixed `sleep(10)`. The temp selfie is deleted in a `finally` so it never lingers in `/tmp` even if Selenium fails (a failed run therefore destroys that photo — re-running just takes a fresh one). Finishes by pruning `SCREENSHOTS_DIR` to `MAX_SCREENSHOTS`.
...
- Shell scripts: POSIX `sh` (not bash), `set -e`, derive `REPO_DIR=$(cd "$(dirname "$0")" && pwd)` and build all paths from it (no hard-coded repo path), `10#$(date +%H)` to force base-10 (avoid leading-zero octal parsing).
- AppleScripts wrap `display dialog` in `try ... on error errNum`, swallowing `-128` (user cancelled). They take the repo dir as `item 1 of argv` (passed by the `.sh` wrapper) and build `run.command` / `snooze.sh` paths from it.
...
- **`activate_safari()` before screenshot is load-bearing.** macOS pauses rendering for background windows, so `driver.save_screenshot()` captures a blank frame unless Safari is foregrounded first ([main.py:100-108](main.py#L100-L108), [main.py:221-222](main.py#L221-L222)). Don't remove it.
...
- **Repo is relocatable.** All source scripts derive their own location (`REPO_DIR=$(cd "$(dirname "$0")" && pwd)`); the `.applescript`s get the repo dir via `argv`. Only the generated `.plist` artifacts embed an absolute path, and `install-schedule.sh` regenerates them from its own location. So moving the repo just means re-running `./install-schedule.sh` — no manual path edits.
...
- **`run.command` upgrades packages on every run** (`pip install --upgrade -r requirements.txt`). Slow but tolerated; don't "fix" without checking with the user. `requirements.txt` now pins exact versions (`==`), so `--upgrade` is effectively a no-op until a pin is bumped.
